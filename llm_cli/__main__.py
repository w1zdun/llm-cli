...
def run(
    args: list[str] = typer.Argument(
        [],
        help="Positional args: @file paths and/or a prompt string.",
    ),
    ...
    from llm_cli.resolve.reasoning import encode_reasoning
    ...
    provider, prov = select_provider(providers_file, provider)
    selected_model = select_model(providers_file, prov, model)
    all_modes = resolve_modes(user_modes)
    ...
    sampling = resolve_sampling(prov, selected_model, mode_obj, cli_sampling)
    extra_body = resolve_extra_body(
        prov, selected_model, mode_obj, cli_overrides
    )
    sampling, extra_body = encode_reasoning(
        selected_model, sampling, extra_body
    )
    ...
    user_content = build_user_message(
        parsed, selected_model, pdf_strategy, max_pages
    )
    messages.append({"role": "user", "content": user_content})

    request_body = build_request(
        prov, selected_model, messages, sampling, extra_body, schema_obj
    )
    ...
    try:
        ...
        if resp.status_code != 200:
            body = resp.text
            typer.echo(
                f"error: HTTP {resp.status_code} from {provider}: {body}",
                err=True,
            )
...
def list_command(
    what: str = typer.Argument(..., help="providers, models, or modes."),
    ...
    """List providers, models, or modes."""
    from llm_cli.config.loader import load_models, load_modes
    ...
    if what == "providers":
        ...
        for name in sorted(all_modes.keys()):
            source = get_mode_source(name, user_modes)
            typer.echo(f"  {name}  ({source})")

    else:
        typer.echo(
            f"error: unknown list target '{what}'; use: providers, models, modes",
            err=True,
        )
        raise typer.Exit(1)
...
@app.command()
def show(
    what: str = typer.Argument(..., help="provider, model, or mode."),
    name: str = typer.Argument(..., help="Name of the item to show."),
    provider_filter: str | None = typer.Option(
        None,
        "--provider",
        "-p",
        help="Provider for model lookup.",
    ),
) -> None:
    """Show full definition of a provider, model, or mode."""
    from llm_cli.config.loader import load_models, load_modes
    from llm_cli.modes.registry import resolve_modes
